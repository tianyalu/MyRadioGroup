package com.sty.myradiogroup.view;
...
import com.sty.myradiogroup.R;

/**
 * Created by shity on 2017/7/24/0024.
 */
...
public class MyRadioGroup extends ViewGroup {
    ...
    public MyRadioGroup(Context context, AttributeSet attrs) {
        ...
        TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.MyRadioGroup) ;
        // 读取默认选中id
        int value = attributes.getResourceId(R.styleable.MyRadioGroup_checkedButton, NO_ID);
        if (value != NO_ID) {
            // 如果为设置checkButton属性，保持默认值NO_ID
            mCheckedId = value;
        }
        // 读取子控件左边距
        childMarginLeft = attributes.getLayoutDimension(R.styleable.MyRadioGroup_childMarginLeft, childMarginLeft);
        if (childMarginLeft < 0) {
            childMarginLeft = 0;
        }
        // 读取子控件右边距
        childMarginRight = attributes.getLayoutDimension(R.styleable.MyRadioGroup_childMarginRight, childMarginRight);
        if (childMarginRight < 0) {
            childMarginRight = 0;
        }
        // 读取子控件上边距
        childMarginTop = attributes.getLayoutDimension(R.styleable.MyRadioGroup_childMarginTop, childMarginTop);
        if (childMarginTop < 0) {
            childMarginTop = 0;
        }
        // 读取子控件下边距
        childMarginBottom = attributes.getLayoutDimension(R.styleable.MyRadioGroup_childMarginBottom, childMarginBottom);
